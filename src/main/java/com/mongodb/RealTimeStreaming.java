...
public class RealTimeStreaming {
	...
	public RealTimeStreaming(String collectionName, String serverName, int socketNumber, String databaseName) throws Exception{
		...
		mongo = new Mongo(serverName, socketNumber);
		db = mongo.getDB(databaseName);
		inputCollection = db.getCollection(collectionName);
	...
	public void realTime() throws Exception{		
		RealTimeStreaming app = new RealTimeStreaming(collectionName, serverName,socketNumber,databaseName);
		try {
			while( true ) {
				...
				if( !app.top10.isEmpty() ) {
					new DataAnalysis("Twitter Stream", today, app.top10);
				}
